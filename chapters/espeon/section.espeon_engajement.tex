...
O processo de cálculo ocorre em etapas. Primeiro, os \textit{traces} de interação são agrupados por aluno e ordenados cronologicamente. Em seguida, para cada par de \textit{traces} consecutivos, é calculada a diferença de tempo em minutos, adicionando-a ao tempo total de sessão. Caso a câmera estivesse ativa no \textit{trace} anterior, esse intervalo também é somado ao tempo de câmera ativa. Assim, obtém-se a proporção individual de tempo com câmera ativa. Por fim, calcula-se a média dessas proporções, resultando no engajamento médio da turma.
...
O processo de cálculo é feito, primeiramente, pelo agrupamento dos \textit{traces} por aluno, ordenados cronologicamente. Em seguida, para cada intervalo entre \textit{traces} consecutivos, é calculada a diferença de tempo em minutos, que é adicionada ao tempo total de sessão. Caso o microfone estivesse ativo no \textit{trace} anterior, esse intervalo também é acumulado como tempo de microfone ativo. Após o cálculo individual das proporções, computa-se a média desses valores, resultando no engajamento médio da turma.
...
O cálculo procede da seguinte forma: inicialmente os \textit{traces} de interação são agrupados por aluno e ordenados cronologicamente; em cada intervalo entre \textit{traces} consecutivos verifica-se se o aluno estava na aba da aula e se havia engajamento multimodal --- isto é, se pelo menos ou câmera ou microfone estão habilitados. Quando atendida, o intervalo é acumulado como tempo de participação voluntária $T_{p,i}$; todos os intervalos contribuem para o tempo total de sessão $T_{s,i}$. Finalmente, calcula-se a razão individual $P_{v,i}$ para cada aluno e obtém-se a média agregada (${P}_v$) da turma.
